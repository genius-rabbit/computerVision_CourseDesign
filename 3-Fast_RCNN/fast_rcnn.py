...
class FAST_RCNN(nn.Module):
    def __init__(self):
        ...
        raw_net = torchvision.models.vgg16_bn(pretrained=False)

        # conv layer sequence
        self.seq = nn.Sequential(*list(raw_net.features.children())[:-1])

        # self.roi_pool = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), dilation=(1, 1))
        ...
        _x = Variable(torch.zeros(1, 3, 128, 128))
        _r = np.array([[0., 0., 1., 1.]])
        _ri = np.array([0])
        # print(self.roi_pool(self.seq(_x), _r, _ri))
        _x = self.feature(self.roi_pool(self.seq(_x), _r, _ri).view(1, -1))
    ...
    def forward(self, imgs, rois, ridx):
        res = self.seq(imgs)
